...
const Post = {
  create: async (userId, postTitle, postDesc) => {
    try {
      ...
      const [result] = await db.execute(
        "INSERT INTO posts (userId, postTitle, postDesc, postDate) VALUES (?, ?, ?, ?)",
        [userId, postTitle, postDesc, postDate]
      );
  ...
  deleteById: async (postId) => {
    try {
      const [result] = await db.execute("DELETE FROM posts WHERE postId = ?", [
        postId,
      ]);
  ...
  like: async (userId, postId) => {
    try {
      const [like] = await db.execute(
        "Select postLikeId FROM likepost where userId= ? AND postId = ?",
        [userId, postId]
      );
      const firstlike = like[0];
      if (like.length > 0) {
        await db.execute("DELETE FROM likepost WHERE postLikeId = ?", [
